...
exports.SWCPropsAnalyzer = void 0;
/**
...
class SWCPropsAnalyzer {
     ...
     */
    async analyzeProps(module, filePath, componentName) {
        const props = [];
     ...
     */
    async resolvePropsTypes(props, filePath, componentName) {
        // Create type query requests for props that have position information
        const typeRequests = props
        ...
        try {
            ...
            for (let i = 0; i < typeRequests.length; i++) {
                const request = typeRequests[i];
                const result = typeResults[i];
                // Find the corresponding prop
                const prop = props.find(p => p.name === request.propName);
                if (prop) {
                    prop.isFunction = result.isFunction;
                    prop.typeString = result.typeString;
                }
            }
        }
        catch (error) {
     ...
     */
    extractPropsFromDeclaration(declaration) {
     ...
     * @returns Array of PropInfo objects
     */
    extractPropsFromObjectPattern(pattern) {
        const props = [];
     ...
     * @returns Array of PropInfo objects
     */
    extractPropsFromIdentifier(identifier) {
        const props = [];
     ...
     * @returns Array of PropInfo objects
     */
    extractPropsFromClass(classDecl) {
        const props = [];
     ...
     */
    extractTypeFromPattern(pattern) {
     ...
     * @returns Type string
     */
    getTypeAnnotationString(typeAnnotation) {
...
exports.SWCPropsAnalyzer = SWCPropsAnalyzer;
