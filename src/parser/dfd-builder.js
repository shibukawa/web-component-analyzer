...
exports.DefaultDFDBuilder = void 0;
/**
 * Default implementation of DFD Builder
 */
class DefaultDFDBuilder {
    nodes = [];
    edges = [];
    nodeIdCounter = 0;
    /**
    ...
    /**
     * Create node for useContext based on classification
     */
    createContextNode(hook) {
     ...
     */
    createStateManagementNode(hook) {
        // For read-write pairs, create a single node
        if (hook.isReadWritePair && hook.variables.length === 2) {
            const [readVar, writeVar] = hook.variables;
            this.nodes.push({
     ...
     */
    createExternalCallNodes(process, processNodeId) {
     ...
     * Infer edges from external entities (props, context, data fetching, routing, server actions) to processes
     */
    inferExternalEntitiesToProcesses(analysis) {
        const externalEntityNodes = this.nodes.filter(node => node.type === 'external-entity-input');
    ...
    /**
     * Infer edges from processes to data stores
     * (when a process calls a state setter function)
     */
    inferProcessesToDataStores(analysis) {
        ...
        for (const process of analysis.processes) {
            ...
            for (const dataStoreNode of dataStoreNodes) {
                // For read-write pairs, check if the write variable is referenced
                ...
                // For custom hooks, check if any write methods are referenced
                    ...
                    const writeMethods = dataStoreNode.metadata.writeMethods;
     ...
     */
    inferDataStoresToProcesses(analysis) {
        const dataStoreNodes = this.nodes.filter(node => node.type === 'data-store');
        for (const process of analysis.processes) {
            const processNode = this.findNodeByLabel(process.name, 'process');
            if (!processNode) {
            ...
            for (const dataStoreNode of dataStoreNodes) {
                if (process.references.includes(dataStoreNode.label)) {
                    this.edges.push({
     ...
     */
    inferDataStoresToJSXOutput(analysis) {
        const dataStoreNodes = this.nodes.filter(node => node.type === 'data-store');
        const jsxNode = this.nodes.find(node => node.type === 'external-entity-output' && node.metadata?.category === 'jsx');
        if (!jsxNode) {
            return;
        }
        ...
        for (const dataStoreNode of dataStoreNodes) {
            const isReferencedInJSX = jsxInfo.placeholders.some(placeholder => placeholder.variables.includes(dataStoreNode.label));
     ...
     * Note: External function call edges are created in createExternalCallNodes
     */
    inferProcessesToExternalOutputs(analysis) {
        const jsxNode = this.nodes.find(node => node.type === 'external-entity-output' && node.metadata?.category === 'jsx');
        ...
        if (jsxNode) {
            for (const process of analysis.processes) {
                ...
                const jsxInfo = analysis.jsxOutput;
                const referencesProcess = jsxInfo.placeholders.some(placeholder => placeholder.variables.includes(process.name));
        ...
        for (const process of analysis.processes) {
            ...
            const functionPropNodes = this.nodes.filter(node => node.type === 'external-entity-output' &&
     ...
     */
    inferInlineCallbackEdges(analysis) {
        const jsxNode = this.nodes.find(node => node.type === 'external-entity-output' && node.metadata?.category === 'jsx');
        if (!jsxNode) {
            return;
        }
        ...
        const inlineCallbackNodes = this.nodes.filter(node => node.type === 'process' && node.label.startsWith('inline_'));
     ...
     */
    inferJSXToCustomHookFunctions(analysis) {
        const jsxNode = this.nodes.find(node => node.type === 'external-entity-output' && node.metadata?.category === 'jsx');
        if (!jsxNode) {
            return;
        }
        ...
        const customHookNodes = this.nodes.filter(node => node.type === 'data-store' && node.metadata?.category === 'custom-hook');
        ...
        for (const customHookNode of customHookNodes) {
            const writeMethods = (customHookNode.metadata?.writeMethods || []);
            for (const writeMethod of writeMethods) {
...
exports.DefaultDFDBuilder = DefaultDFDBuilder;
